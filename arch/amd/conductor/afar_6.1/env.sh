...
export HDF5_DIR=/PATH/TO/HDF5_BUILT_WITH_ROCM_AFAR_6.1.0/
if [[ ! -d $HDF5_DIR ]]; then
    echo "You must set the HDF5 directory to one built with rocm-afar-drop/6.1.0"
    echo "You must also set the paths to the rocm-afar-drop/6.1.0 compiler correctly"
    exit
fi
