...
export PATH=/ccs/home/mullowne/software/cmake-3.28.3/bin:$PATH
which cmake
...
then
	 ...
	 if [[ $use_ecbundle ]];
	 then
		  ./cloudsc-bundle build --clean --cmake="OpenMP_C_LIB_NAMES=craymp OpenMP_CXX_LIB_NAMES=craymp OpenMP_Fortran_LIB_NAMES=craymp OpenMP_craymp_LIBRARY=/opt/cray/pe/cce/${cce}/cce/x86_64/lib/libcraymp.so OpenMP_C_FLAGS=-fopenmp OpenMP_CXX_FLAGS=-fopenmp OpenMP_Fortran_FLAGS='-homp -hlist=aimd'" --arch arch/olcf/${machine}/cray-gpu/${cce} --build-dir=${build_dir}_${cce} --with-gpu -j=${ranks}
