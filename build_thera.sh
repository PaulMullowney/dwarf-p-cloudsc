...
then
	...
	then	
	    ...
	    ./cloudsc-bundle build --clean --build-dir=${bdir} --arch=arch/${machine}/amd_staging -j=${ranks} --cmake="CMAKE_EXE_LINKER_FLAGS_INIT=-lflang_rt.hostdevice" --with-serialbox=${serialbox} --with-hip=OFF --with-acc=OFF --with-openmp=ON --with-single-precision=${sp}
	else
	    echo "Build AFAR"
	    bdir=${build_dir}_afar_${afar}
	    ./cloudsc-bundle build --clean --build-dir=${bdir} --arch=arch/${machine}/afar_${afar} -j=${ranks} --cmake="CMAKE_EXE_LINKER_FLAGS_INIT='-lflang_rt.hostdevice -fopenmp --offload-arch=${arch}'" --with-serialbox=${serialbox} --with-hip=OFF --with-acc=OFF --with-openmp=ON --with-single-precision=${sp}
	fi
    fi
fi
