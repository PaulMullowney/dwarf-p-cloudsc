...
then
	...
	then	
	    ...
	    ./cloudsc-bundle build -v --clean --build-dir=${bdir} --arch=arch/${machine}/afar_${afar} -j=${ranks} --cmake="CMAKE_EXE_LINKER_FLAGS_INIT=-lflang_rt.hostdevice GPU_STATIC=ON" --with-serialbox=${serialbox} --with-hip=OFF --with-acc=OFF --with-openmp=ON --with-single-precision=${sp}
	fi
    fi
fi
