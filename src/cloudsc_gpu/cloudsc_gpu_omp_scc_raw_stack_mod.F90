...
MODULE CLOUDSC_GPU_OMP_SCC_RAW_STACK_MOD

!$omp declare target(cloudsc_scc_raw_stack)

  CONTAINS
  
  SUBROUTINE CLOUDSC_SCC_RAW_STACK (KIDIA, KFDIA, KLON, KLEV, PTSPHY, PT, PQ, TENDENCY_TMP_T, TENDENCY_TMP_Q, TENDENCY_TMP_A,  &
  ...
  & PFCQSNG, PFSQLTUR, PFSQITUR, PFPLSL, PFPLSN, PFHPSL, PFHPSN, YRECLDP, ZSTACK, JL, ISTSZ)
    !---input
    !---prognostic fields
    ...
    INTEGER(KIND=8), INTENT(IN) :: ISTSZ
    REAL(KIND=JPRB), INTENT(INOUT) :: ZSTACK(ISTSZ)
    
    INTEGER(KIND=8) :: IND_ZFOEALFA, IND_ZTP1, IND_ZLI, IND_ZA, IND_ZAORIG, &
         IND_ZLIQFRAC, IND_ZICEFRAC, IND_ZQX, IND_ZQX0, IND_ZPFPLSX, IND_ZLNEG, &
         IND_ZQXN2D, IND_ZQSMIX, IND_ZQSLIQ, IND_ZQSICE, IND_ZFOEEWMT, IND_ZFOEEW, &
         IND_ZFOEELIQT
...
#include "fccld.func.h"
    ...
    !REAL(KIND=JPRB) :: ZFOEELIQT(KLON, KLEV)
    IND_ZFOEALFA = 0
    IND_ZTP1 =      KLON*(KLEV + 1) + IND_ZFOEALFA
    IND_ZLI =       KLON*KLEV + IND_ZTP1
    IND_ZA =        KLON*KLEV + IND_ZLI
    IND_ZAORIG =    KLON*KLEV + IND_ZA
    IND_ZLIQFRAC =  KLON*KLEV + IND_ZAORIG
    IND_ZICEFRAC =  KLON*KLEV + IND_ZLIQFRAC
    IND_ZQX =       KLON*KLEV + IND_ZICEFRAC
    IND_ZQX0 =      KLON*KLEV*NCLV + IND_ZQX
    IND_ZPFPLSX =   KLON*KLEV*NCLV + IND_ZQX0
    IND_ZLNEG =     KLON*(KLEV + 1)*NCLV + IND_ZPFPLSX
    IND_ZQXN2D =    KLON*KLEV*NCLV + IND_ZLNEG
    IND_ZQSMIX =    KLON*KLEV*NCLV + IND_ZQXN2D
    IND_ZQSLIQ =    KLON*KLEV + IND_ZQSMIX
    IND_ZQSICE =    KLON*KLEV + IND_ZQSLIQ
    IND_ZFOEEWMT =  KLON*KLEV + IND_ZQSICE
    IND_ZFOEEW =    KLON*KLEV + IND_ZFOEEWMT
    IND_ZFOEELIQT = KLON*KLEV + IND_ZFOEEW
      ...
      DO JK=1,KLEV
        ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) = PT(JL, JK) + PTSPHY*TENDENCY_TMP_T(JL, JK)
        ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) = PQ(JL, JK) + PTSPHY*TENDENCY_TMP_Q(JL, JK)
        ZSTACK(IND_ZQX0 + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) = PQ(JL, JK) + PTSPHY*TENDENCY_TMP_Q(JL, JK)
        ZSTACK(IND_ZA + (JK-1)*KLON + JL) = PA(JL, JK) + PTSPHY*TENDENCY_TMP_A(JL, JK)
        ZSTACK(IND_ZAORIG + (JK-1)*KLON + JL) = PA(JL, JK) + PTSPHY*TENDENCY_TMP_A(JL, JK)
      END DO
      ...
      DO JM=1,NCLV - 1
        ...
        DO JK=1,KLEV
          ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) = PCLV(JL, JK, JM) + PTSPHY*TENDENCY_TMP_CLD(JL, JK, JM)
          ZSTACK(IND_ZQX0 + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) = PCLV(JL, JK, JM) + PTSPHY*TENDENCY_TMP_CLD(JL, JK, JM)
        END DO
      END DO
      ...
      DO JM=1,NCLV
        ...
        DO JK=1,KLEV + 1
          ZSTACK(IND_ZPFPLSX + (JM-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL) = 0.0_JPRB            ! precip fluxes
        END DO
      END DO
      ...
      DO JM=1,NCLV
        ...
        DO JK=1,KLEV
          ZSTACK(IND_ZQXN2D + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) = 0.0_JPRB            ! end of timestep values in 2D
          ZSTACK(IND_ZLNEG + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) = 0.0_JPRB            ! negative input check
        END DO
      END DO
      ...
      DO JK=1,KLEV
        IF (ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL) < YRECLDP%RLMIN .or. ZSTACK(IND_ZA + (JK-1)*KLON + JL) < YRECLDP%RAMIN) THEN
          
          ! Evaporate small cloud liquid water amounts
          ZSTACK(IND_ZLNEG + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) = ZSTACK(IND_ZLNEG + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL)
          ZQADJ = ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZQTMST
          TENDENCY_LOC_Q(JL, JK) = TENDENCY_LOC_Q(JL, JK) + ZQADJ
          TENDENCY_LOC_T(JL, JK) = TENDENCY_LOC_T(JL, JK) - RALVDCP*ZQADJ
          ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) = ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL)
          ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) = 0.0_JPRB
          
          ! Evaporate small cloud ice water amounts
          ZSTACK(IND_ZLNEG + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL) = ZSTACK(IND_ZLNEG + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)
          ZQADJ = ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZQTMST
          TENDENCY_LOC_Q(JL, JK) = TENDENCY_LOC_Q(JL, JK) + ZQADJ
          TENDENCY_LOC_T(JL, JK) = TENDENCY_LOC_T(JL, JK) - RALSDCP*ZQADJ
          ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) = ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)
          ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL) = 0.0_JPRB
          
          ! Set cloud cover to zero
          ZSTACK(IND_ZA + (JK-1)*KLON + JL) = 0.0_JPRB
      ...
      DO JM=1,NCLV - 1
        ...
        DO JK=1,KLEV
          !DIR$ IVDEP
          IF (ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) < YRECLDP%RLMIN) THEN
            ZSTACK(IND_ZLNEG + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) = ZSTACK(IND_ZLNEG + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL)
            ZQADJ = ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZQTMST
            TENDENCY_LOC_Q(JL, JK) = TENDENCY_LOC_Q(JL, JK) + ZQADJ
            ...
            ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) = ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL)
            ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) = 0.0_JPRB
          END IF
        END DO
      END DO
      ...
      DO JK=1,KLEV
        ...
        !----------------------------------------
        ZSTACK(IND_ZFOEALFA + (JK-1)*KLON + JL) = FOEALFA(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))
        ZSTACK(IND_ZFOEEWMT + (JK-1)*KLON + JL) = MIN(FOEEWM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)) / PAP(JL, JK), 0.5_JPRB)
        ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) = ZSTACK(IND_ZFOEEWMT + (JK-1)*KLON + JL)
        ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) = ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) / (1.0_JPRB - RETV*ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL))
        
        !---------------------------------------------
        ! ice saturation T<273K
        ! liquid water saturation for T>273K
        !---------------------------------------------
        ZALFA = FOEDELTA(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))
        ZSTACK(IND_ZFOEEW + (JK-1)*KLON + JL) = MIN((ZALFA*FOEELIQ(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)) + (1.0_JPRB - ZALFA)*FOEEICE(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))) / PAP(JL, JK), 0.5_JPRB)
        ZSTACK(IND_ZFOEEW + (JK-1)*KLON + JL) = MIN(0.5_JPRB, ZSTACK(IND_ZFOEEW + (JK-1)*KLON + JL))
        ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) = ZSTACK(IND_ZFOEEW + (JK-1)*KLON + JL) / (1.0_JPRB - RETV*ZSTACK(IND_ZFOEEW + (JK-1)*KLON + JL))
        
        !----------------------------------
        ! liquid water saturation
        !----------------------------------
        ZSTACK(IND_ZFOEELIQT + (JK-1)*KLON + JL) = MIN(FOEELIQ(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)) / PAP(JL, JK), 0.5_JPRB)
        ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL) = ZSTACK(IND_ZFOEELIQT + (JK-1)*KLON + JL)
        ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL) = ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL) / (1.0_JPRB - RETV*ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL))
        
        !   !----------------------------------
        !   ! ice water saturation
        !   !----------------------------------
        !   ZFOEEICET(JL,JK)=MIN(FOEEICE(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))/PAP(JL,JK),0.5_JPRB)
        !   ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)=ZFOEEICET(JL,JK)
        !   ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)=ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)/(1.0_JPRB-RETV*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL))
      ...
      DO JK=1,KLEV
        ...
        !------------------------------------------
        ZSTACK(IND_ZA + (JK-1)*KLON + JL) = MAX(0.0_JPRB, MIN(1.0_JPRB, ZSTACK(IND_ZA + (JK-1)*KLON + JL)))
        
        !-------------------------------------------------------------------
        ! Calculate liq/ice fractions (no longer a diagnostic relationship)
        !-------------------------------------------------------------------
        ZSTACK(IND_ZLI + (JK-1)*KLON + JL) = ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)
        IF (ZSTACK(IND_ZLI + (JK-1)*KLON + JL) > YRECLDP%RLMIN) THEN
          ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL) = ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) / ZSTACK(IND_ZLI + (JK-1)*KLON + JL)
          ZSTACK(IND_ZICEFRAC + (JK-1)*KLON + JL) = 1.0_JPRB - ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL)
        ELSE
          ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL) = 0.0_JPRB
          ZSTACK(IND_ZICEFRAC + (JK-1)*KLON + JL) = 0.0_JPRB
        END IF
      ...
      DO JK=1,KLEV - 1
        ...
        IF (ZSIG > 0.1_JPRB .and. ZSIG < 0.4_JPRB .and. ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) > ZSTACK(IND_ZTP1 + (1+JK-1)*KLON + JL)) THEN
          ZTRPAUS = ZSIG
        END IF
      END DO
      ...
      DO JK=YRECLDP%NCLDTOP,KLEV
        ...
        DO JM=1,NCLV
          ZQXFG(JM) = ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL)
        END DO
        ...
        ZGDP = RG / ZDP          ! g/dp
        ZRHO = PAP(JL, JK) / (RD*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))          ! p/RT air density
        ...
        ! liquid
        ZFACW = R5LES / ((ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) - R4LES)**2)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZSTACK(IND_ZFOEELIQT + (JK-1)*KLON + JL))
        ZDQSLIQDT = ZFACW*ZCOR*ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL)
        ZCORQSLIQ = 1.0_JPRB + RALVDCP*ZDQSLIQDT
        
        ! ice
        ZFACI = R5IES / ((ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) - R4IES)**2)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZSTACK(IND_ZFOEEW + (JK-1)*KLON + JL))
        ZDQSICEDT = ZFACI*ZCOR*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)
        ZCORQSICE = 1.0_JPRB + RALSDCP*ZDQSICEDT
        
        ! diagnostic mixed
        ZALFAW = ZSTACK(IND_ZFOEALFA + (JK-1)*KLON + JL)
        ZALFAWM = ZALFAW
        ZFAC = ZALFAW*ZFACW + (1.0_JPRB - ZALFAW)*ZFACI
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZSTACK(IND_ZFOEEWMT + (JK-1)*KLON + JL))
        ZDQSMIXDT = ZFAC*ZCOR*ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL)
        ZCORQSMIX = 1.0_JPRB + FOELDCPM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*ZDQSMIXDT
        
        ! evaporation/sublimation limits
        ZEVAPLIMMIX = MAX((ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL)) / ZCORQSMIX, 0.0_JPRB)
        ZEVAPLIMLIQ = MAX((ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL)) / ZCORQSLIQ, 0.0_JPRB)
        ZEVAPLIMICE = MAX((ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL)) / ZCORQSICE, 0.0_JPRB)
        
        !--------------------------------
        ! in-cloud consensate amount
        !--------------------------------
        ZTMPA = 1.0_JPRB / MAX(ZSTACK(IND_ZA + (JK-1)*KLON + JL), ZEPSEC)
        ZLIQCLD = ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZTMPA
        ZICECLD = ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZTMPA
        ...
        IF (ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) < YRECLDP%RLMIN) THEN
          ZSOLQA(NCLDQV, NCLDQL) = ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL)
          ZSOLQA(NCLDQL, NCLDQV) = -ZSTACK(IND_ZQX + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL)
        END IF
        
        IF (ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL) < YRECLDP%RLMIN) THEN
          ZSOLQA(NCLDQV, NCLDQI) = ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)
          ZSOLQA(NCLDQI, NCLDQV) = -ZSTACK(IND_ZQX + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)
        END IF
        ...
        ZFOKOOP = FOKOOP(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))
        
        IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) >= RTT .or. YRECLDP%NSSOPT == 0) THEN
          ZFAC = 1.0_JPRB
          ZFACI = 1.0_JPRB
        ELSE
          ZFAC = ZSTACK(IND_ZA + (JK-1)*KLON + JL) + ZFOKOOP*(1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))
        ...
        IF (ZSTACK(IND_ZA + (JK-1)*KLON + JL) > 1.0_JPRB - YRECLDP%RAMIN) THEN
          ZSUPSAT = MAX((ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZFAC*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)) / ZCORQSICE, 0.0_JPRB)
        ELSE
          ! Calculate environmental humidity supersaturation
          ZQP1ENV = (ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)) / MAX(1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL), ZEPSILON)
          !& SIGN(MAX(ABS(1.0_JPRB-ZSTACK(IND_ZA + (JK-1)*KLON + JL)),ZEPSILON),1.0_JPRB-ZSTACK(IND_ZA + (JK-1)*KLON + JL))
          ZSUPSAT = MAX((1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))*(ZQP1ENV - ZFAC*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)) / ZCORQSICE, 0.0_JPRB)
        END IF
        ...
        IF (ZSUPSAT > ZEPSEC) THEN
          ...
          IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) > YRECLDP%RTHOMO) THEN
          ...
          ZSOLAC = (1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))*ZFACI
        ...
        IF (JK > YRECLDP%NCLDTOP) THEN
          ...
          ZDTDP = ZRDCP*0.5_JPRB*(ZSTACK(IND_ZTP1 + (JK-1-1)*KLON + JL) + ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)) / PAPH(JL, JK)
        ...
        IF (ZSTACK(IND_ZLI + (JK-1)*KLON + JL) > ZEPSEC) THEN
          ! Calculate environmental humidity
          !      ZQE=(ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL)-ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL))/&
          !    &      MAX(ZEPSEC,1.0_JPRB-ZSTACK(IND_ZA + (JK-1)*KLON + JL))
          !      ZE=ZLDIFDT(JL)*MAX(ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL)-ZQE,0.0_JPRB)
          ZE = ZLDIFDT*MAX(ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL), 0.0_JPRB)
          ZLEROS = ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZE
          ZLEROS = MIN(ZLEROS, ZEVAPLIMMIX)
          ZLEROS = MIN(ZLEROS, ZSTACK(IND_ZLI + (JK-1)*KLON + JL))
          ZAEROS = ZLEROS / ZLICLD            !if linear term
          
          ! Erosion is -ve LINEAR in L,A
          ZSOLAC = ZSOLAC - ZAEROS            !linear
          
          ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL)*ZLEROS
          ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL)*ZLEROS
          ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + ZSTACK(IND_ZICEFRAC + (JK-1)*KLON + JL)*ZLEROS
          ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - ZSTACK(IND_ZICEFRAC + (JK-1)*KLON + JL)*ZLEROS
        ...
        ZDTDP = ZRDCP*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) / PAP(JL, JK)
        ZDPMXDT = ZDP*ZQTMST
        ZMFDN = 0.0_JPRB
        ...
        ZQOLD = ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL)
        ZTOLD = ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)
        ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) = ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + ZDTFORC
        ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) = MAX(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL), 160.0_JPRB)
        LLFLAG = .true.
        
        ! Formerly a call to CUADJTQ(..., ICALL=5)
        ZQP = 1.0_JPRB / PAP(JL, JK)
        ZQSAT = FOEEWM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*ZQP
        ZQSAT = MIN(0.5_JPRB, ZQSAT)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
        ZQSAT = ZQSAT*ZCOR
        ZCOND = (ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)))
        ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) = ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + FOELDCPM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*ZCOND
        ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) = ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZCOND
        ZQSAT = FOEEWM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*ZQP
        ZQSAT = MIN(0.5_JPRB, ZQSAT)
        ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZQSAT)
        ZQSAT = ZQSAT*ZCOR
        ZCOND1 = (ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZQSAT) / (1.0_JPRB + ZQSAT*ZCOR*FOEDEM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)))
        ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) = ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + FOELDCPM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*ZCOND1
        ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) = ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZCOND1
        
        ZDQS = ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZQOLD
        ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) = ZQOLD
        ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) = ZTOLD
        ...
        IF (ZDQS > 0.0_JPRB) THEN
          ...
          !    ZLEVAP = MAX(ZSTACK(IND_ZA + (JK-1)*KLON + JL)+ZACUST(JL),1.0_JPRB)*MIN(ZDQS(JL),ZLICLD(JL)+ZLFINALSUM(JL))
          ZLEVAP = ZSTACK(IND_ZA + (JK-1)*KLON + JL)*MIN(ZDQS, ZLICLD)
          ZLEVAP = MIN(ZLEVAP, ZEVAPLIMMIX)
          ZLEVAP = MIN(ZLEVAP, MAX(ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL), 0.0_JPRB))
          
          ! For first guess call
          ZLEVAPL = ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL)*ZLEVAP
          ZLEVAPI = ZSTACK(IND_ZICEFRAC + (JK-1)*KLON + JL)*ZLEVAP
          
          ZSOLQA(NCLDQV, NCLDQL) = ZSOLQA(NCLDQV, NCLDQL) + ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL)*ZLEVAP
          ZSOLQA(NCLDQL, NCLDQV) = ZSOLQA(NCLDQL, NCLDQV) - ZSTACK(IND_ZLIQFRAC + (JK-1)*KLON + JL)*ZLEVAP
          
          ZSOLQA(NCLDQV, NCLDQI) = ZSOLQA(NCLDQV, NCLDQI) + ZSTACK(IND_ZICEFRAC + (JK-1)*KLON + JL)*ZLEVAP
          ZSOLQA(NCLDQI, NCLDQV) = ZSOLQA(NCLDQI, NCLDQV) - ZSTACK(IND_ZICEFRAC + (JK-1)*KLON + JL)*ZLEVAP
        ...
        IF (ZSTACK(IND_ZA + (JK-1)*KLON + JL) > ZEPSEC .and. ZDQS <= -YRECLDP%RLMIN) THEN
          
          ZLCOND1 = MAX(-ZDQS, 0.0_JPRB)            !new limiter
          
          !old limiter (significantly improves upper tropospheric humidity rms)
          IF (ZSTACK(IND_ZA + (JK-1)*KLON + JL) > 0.99_JPRB) THEN
            ZCOR = 1.0_JPRB / (1.0_JPRB - RETV*ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL))
            ZCDMAX = (ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL)) / (1.0_JPRB + ZCOR*ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL)*FOEDEM(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)))
          ELSE
            ZCDMAX = (ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZSTACK(IND_ZQSMIX + (JK-1)*KLON + JL)) / ZSTACK(IND_ZA + (JK-1)*KLON + JL)
          END IF
          ZLCOND1 = MAX(MIN(ZLCOND1, ZCDMAX), 0.0_JPRB)
          ! end old limiter
          
          ZLCOND1 = ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZLCOND1
        ...
        IF (ZDQS <= -YRECLDP%RLMIN .and. ZSTACK(IND_ZA + (JK-1)*KLON + JL) < 1.0_JPRB - ZEPSEC) THEN
          ...
          IF (YRECLDP%NSSOPT == 0) THEN
            ! No scheme
            ZQE = (ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)) / MAX(ZEPSEC, 1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))
          ...
          ELSE IF (YRECLDP%NSSOPT == 2) THEN
            ! Lohmann and Karcher
            ZQE = ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL)
          ELSE IF (YRECLDP%NSSOPT == 3) THEN
            ! Gierens
            ZQE = ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZLI + (JK-1)*KLON + JL)
          END IF
          ...
          IF (ZQE >= ZRHC*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)*ZFAC .and. ZQE < ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)*ZFAC) THEN
            ! note: not **2 on 1-a term if ZQE is used.
            ! Added correction term ZFAC to numerator 15/03/2010
            ZACOND = -(1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))*ZFAC*ZDQS / MAX(2.0_JPRB*(ZFAC*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) - ZQE), ZEPSEC)
            
            ZACOND = MIN(ZACOND, 1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))              !PUT THE LIMITER BACK
            ...
            ZZDL = 2.0_JPRB*(ZFAC*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) - ZQE) / MAX(ZEPSEC, 1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))
            ! Added correction term ZFAC 15/03/2010
            IF (ZFAC*ZDQS < -ZZDL) THEN
              ! ZLCONDLIM=(ZSTACK(IND_ZA + (JK-1)*KLON + JL)-1.0_JPRB)*ZDQS(JL)-ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)+ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL)
              ZLCONDLIM = (ZSTACK(IND_ZA + (JK-1)*KLON + JL) - 1.0_JPRB)*ZFAC*ZDQS - ZFAC*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL)
              ZLCOND2 = MIN(ZLCOND2, ZLCONDLIM)
            END IF
            ZLCOND2 = MAX(ZLCOND2, 0.0_JPRB)
            
            IF (ZLCOND2 < YRECLDP%RLMIN .or. 1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL) < ZEPSEC) THEN
              ZLCOND2 = 0.0_JPRB
              ZACOND = 0.0_JPRB
            END IF
        ...
        IF (IDEPICE == 1) THEN
          ...
          IF (ZSTACK(IND_ZA + (-1+JK-1)*KLON + JL) < YRECLDP%RCLDTOPCF .and. ZSTACK(IND_ZA + (JK-1)*KLON + JL) >= YRECLDP%RCLDTOPCF) THEN
            ZCLDTOPDIST = 0.0_JPRB
          ELSE
          ...
          !--------------------------------------------------------------
          IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
            ! T<273K
            
            ZVPICE = FOEEICE(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*RV / RD
            ...
            !------------------------------------------------
            ZADD = RLSTT*(RLSTT / (RV*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)) - 1.0_JPRB) / (2.4E-2_JPRB*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))
            ZBDD = RV*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)*PAP(JL, JK) / (2.21_JPRB*ZVPICE)
            ...
            !---------------------------
            ZDEPOS = MAX(ZSTACK(IND_ZA + (JK-1)*KLON + JL)*(ZINEW - ZICE0), 0.0_JPRB)
        ...
        ELSE IF (IDEPICE == 2) THEN
          ...
          IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) < RTT .and. ZQXFG(NCLDQL) > YRECLDP%RLMIN) THEN
            ...
            ZAPLUSB =  &
            & YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + PAP(JL, JK)*YRECLDP%RCL_APB3*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)**3._JPRB
            ZCORRFAC = (1.0_JPRB / ZRHO)**0.5_JPRB
            ZCORRFAC2 = ((ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) / 273.0_JPRB)**1.5_JPRB)*(393.0_JPRB / (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + 120.0_JPRB))
            
            ZPR02 = ZRHO*ZICE0*YRECLDP%RCL_CONST1I / (ZTCG*ZFACX1I)
            
            ZTERM1 =  &
            & (ZVPLIQ - ZVPICE)*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)**2.0_JPRB*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2I*ZFACX1I / (ZRHO*ZAPLUSB*ZVPICE)
            ...
            ZDEPOS = MAX(ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZTERM1*ZTERM2*PTSPHY, 0.0_JPRB)
        ...
        DO JM=1,NCLV
          IF (LLFALL(JM) .or. JM == NCLDQI) THEN
            ...
            !------------------------
            IF (JK > YRECLDP%NCLDTOP) THEN
              ZFALLSRCE(JM) = ZSTACK(IND_ZPFPLSX + (JM-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL)*ZDTGDP
            ...
            !            &*((ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)*RICEHI2)**(-0.394_JPRB))
        ...
        IF (ZQPRETOT > ZEPSEC) THEN
           ZCOVPTOT = 1.0_JPRB - ((1.0_JPRB - ZCOVPTOT)*(1.0_JPRB - MAX(ZSTACK(IND_ZA + (JK-1)*KLON + JL), ZSTACK(IND_ZA + (JK-1-1)*KLON + JL))) / (1.0_JPRB - &
                MIN(ZSTACK(IND_ZA + (JK-2)*KLON +JL), 1.0_JPRB - 1.E-06_JPRB)))
          ZCOVPTOT = MAX(ZCOVPTOT, YRECLDP%RCOVPMIN)
          ZCOVPCLR = MAX(0.0_JPRB, ZCOVPTOT - ZSTACK(IND_ZA + (JK-1)*KLON + JL))            ! clear sky proportion
        ...
        IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) <= RTT) THEN
          !-----------------------------------------------------
          !     Snow Autoconversion rate follow Lin et al. 1983
          !-----------------------------------------------------
          IF (ZICECLD > ZEPSEC) THEN
            
            ZZCO = PTSPHY*YRECLDP%RSNOWLIN1*EXP(YRECLDP%RSNOWLIN2*(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) - RTT))
        ...
        IF (ZLIQCLD > ZEPSEC) THEN
          ...
          IF (IWARMRAIN == 1) THEN
            ...
            !------------------------------------------------------------------
            ZPRECIP = (ZSTACK(IND_ZPFPLSX + (NCLDQS-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL) + ZSTACK(IND_ZPFPLSX + (NCLDQR-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL)) / MAX(ZEPSEC, ZCOVPTOT)
            ZCFPR = 1.0_JPRB + YRECLDP%RPRC1*SQRT(MAX(ZPRECIP, 0.0_JPRB))
            !      ZCFPR=1.0_JPRB + RPRC1*SQRT(MAX(ZPRECIP,0.0_JPRB))*&
            !       &ZCOVPTOT(JL)/(MAX(ZSTACK(IND_ZA + (JK-1)*KLON + JL),ZEPSEC))
          ...
          ELSE IF (IWARMRAIN == 2) THEN
            ...
            IF (ZLIQCLD > ZLCRIT) THEN
              ...
              ZRAINAUT = 1.5_JPRB*ZSTACK(IND_ZA + (JK-1)*KLON + JL)*PTSPHY*YRECLDP%RCL_KKAAU*ZLIQCLD**YRECLDP%RCL_KKBAUQ*ZCONST**YRECLDP%RCL_KKBAUN
              
              ZRAINAUT = MIN(ZRAINAUT, ZQXFG(NCLDQL))
              IF (ZRAINAUT < ZEPSEC) ZRAINAUT = 0.0_JPRB
              
              ZRAINACC = 2.0_JPRB*ZSTACK(IND_ZA + (JK-1)*KLON + JL)*PTSPHY*YRECLDP%RCL_KKAAC*(ZLIQCLD*ZRAINCLD)**YRECLDP%RCL_KKBAC
        ...
        IF (IWARMRAIN > 1) THEN
          ...
          IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) <= RTT .and. ZLIQCLD > ZEPSEC) THEN
            ...
            ! NOT YET ACTIVE
            !------------------------------------------------------------------
            !      IF (ZICECLD(JL)>ZEPSEC .AND. ZSTACK(IND_ZA + (JK-1)*KLON + JL)>0.01_JPRB) THEN
            !
            !        ! Calculate riming term
            !        ! Factor of liq water taken out because implicit
            !        ZSNOWRIME(JL) = ZSTACK(IND_ZA + (JK-1)*KLON + JL)*PTSPHY*RCL_CONST7S*ZFALLCORR &
        ...
        IF (ZICETOT > ZEPSEC .and. ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) > RTT) THEN
          
          ! Calculate subsaturation
          ZSUBSAT = MAX(ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL), 0.0_JPRB)
          ...
          ! due to evaporation.
          ZTDMTW0 = ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) - RTT - ZSUBSAT*(ZTW1 + ZTW2*(PAP(JL, JK) - ZTW3) - ZTW4*(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) - ZTW5))
          ! Not implicit yet...
        ...
        ! If rain present
        IF (ZSTACK(IND_ZQX + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL) > ZEPSEC) THEN
          
          IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) <= RTT .and. ZSTACK(IND_ZTP1 + (-1+JK-1)*KLON + JL) > RTT) THEN
            ...
            ZQPRETOT = MAX(ZSTACK(IND_ZQX + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZSTACK(IND_ZQX + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL), ZEPSEC)
            PRAINFRAC_TOPRFZ(JL) = ZSTACK(IND_ZQX + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL) / ZQPRETOT
          ...
          IF (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) < RTT) THEN
            ...
            IF (PRAINFRAC_TOPRFZ(JL) > 0.8) THEN
              ...
              ZLAMBDA = (YRECLDP%RCL_FAC1 / (ZRHO*ZSTACK(IND_ZQX + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL)))**YRECLDP%RCL_FAC2
              
              ! Calculate freezing rate based on Bigg(1953) and Wisner(1972)
              ZTEMP = YRECLDP%RCL_FZRAB*(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) - RTT)
            ...
            ELSE
              ...
              ZCONS1 = ABS(PTSPHY*(1.0_JPRB + 0.5_JPRB*(RTT - ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))) / YRECLDP%RTAUMEL)
              ZFRZMAX = MAX((RTT - ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*ZCONS1*ZRLDCP, 0.0_JPRB)
              
            END IF
            
            IF (ZFRZMAX > ZEPSEC) THEN
              ZFRZ = MIN(ZSTACK(IND_ZQX + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL), ZFRZMAX)
        ...
        !----------------------------------------------------------------------
        ! not implicit yet...
        ZFRZMAX = MAX((YRECLDP%RTHOMO - ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*ZRLDCP, 0.0_JPRB)
        ...
        IF (IEVAPRAIN == 1) THEN
          ...
          ZZRH = YRECLDP%RPRECRHMAX + (1.0_JPRB - YRECLDP%RPRECRHMAX)*ZCOVPMAX / MAX(ZEPSEC, 1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))
          ZZRH = MIN(MAX(ZZRH, YRECLDP%RPRECRHMAX), 1.0_JPRB)
          
          ZQE = (ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZA + (JK-1)*KLON + JL)*ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL)) / MAX(ZEPSEC, 1.0_JPRB - ZSTACK(IND_ZA + (JK-1)*KLON + JL))
          !---------------------------------------------
          ! humidity in moistest ZCOVPCLR part of domain
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQR) > ZEPSEC .and. ZQE < ZZRH*ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL)
          ...
          IF (LLO1) THEN
            ...
            ZDPR = ZCOVPCLR*ZBETA*(ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL) - ZQE) / ZDENOM*ZDP*ZRG_R
            ZDPEVAP = ZDPR*ZDTGDP
            ...
            !-------------------------------------------------------------
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - ZSTACK(IND_ZA + (JK-1)*KLON + JL))*ZEVAP / ZQXFG(NCLDQR)))
        ...
        ELSE IF (IEVAPRAIN == 2) THEN
          ...
          ZQE = MAX(0.0_JPRB, MIN(ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL), ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL)))
          ...
          IF (LLO1) THEN
            ...
            ZESATLIQ = RV / RD*FOEELIQ(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))
            
            ! Slope of particle size distribution
            ZLAMBDA = (YRECLDP%RCL_FAC1 / (ZRHO*ZPRECLR))**YRECLDP%RCL_FAC2              ! ZPRECLR=kg/kg
            
            ZEVAP_DENOM = YRECLDP%RCL_CDENOM1*ZESATLIQ - YRECLDP%RCL_CDENOM2*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)*ZESATLIQ + YRECLDP%RCL_CDENOM3* &
                 ZSTACK(IND_ZTP1 + (JK-1)*KLON +JL)**3._JPRB*PAP(JL, JK)
            
            ! Temperature dependent conductivity
            ZCORR2 = (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) / 273._JPRB)**1.5_JPRB*393._JPRB / (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + 120._JPRB)
            ZKA = YRECLDP%RCL_KA273*ZCORR2
            
            ZSUBSAT = MAX(ZZRH*ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL) - ZQE, 0.0_JPRB)
            
            ZBETA = (0.5_JPRB / ZSTACK(IND_ZQSLIQ + (JK-1)*KLON + JL))*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)**2._JPRB*ZESATLIQ*YRECLDP%RCL_CONST1R*(ZCORR2 / ZEVAP_DENOM) &
        ...
        IF (IEVAPSNOW == 1) THEN
          ...
          ! humidity in moistest ZCOVPCLR part of domain
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZQXFG(NCLDQS) > ZEPSEC .and. ZQE < ZZRH*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)
          ...
          IF (LLO1) THEN
            ...
            ZDPR = ZCOVPCLR*ZBETA*(ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) - ZQE) / ZDENOM*ZDP*ZRG_R
            ZDPEVAP = ZDPR*ZDTGDP
            ...
            ! 2-flux treatment, abandoned due to the new prognostic precip
            !-------------------------------------------------------------
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - ZSTACK(IND_ZA + (JK-1)*KLON + JL))*ZEVAP / ZQXFG(NCLDQS)))
        ...
        ELSE IF (IEVAPSNOW == 2) THEN
          ...
          !---------------------------------------------
          ZQE = MAX(0.0_JPRB, MIN(ZQE, ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)))
          LLO1 = ZCOVPCLR > ZEPSEC .and. ZSTACK(IND_ZQX + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL) > ZEPSEC .and. ZQE < ZZRH*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL)
          
          IF (LLO1) THEN
            
            ! Calculate local precipitation (kg/kg)
            ZPRECLR = ZSTACK(IND_ZQX + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL) / ZCOVPTOT
            ZVPICE = FOEEICE(ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))*RV / RD
            
            ! Particle size distribution
            ! ZTCG increases Ni with colder temperatures - essentially a
            ! Fletcher or Meyers scheme?
            ZTCG = 1.0_JPRB              !v1 EXP(RCL_X3I*(273.15_JPRB-ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL))/8.18_JPRB)
            ! ZFACX1I modification is based on Andrew Barrett's results
            ZFACX1S = 1.0_JPRB              !v1 (ZICE0/1.E-5_JPRB)**0.627_JPRB
            
            ZAPLUSB =  &
            & YRECLDP%RCL_APB1*ZVPICE - YRECLDP%RCL_APB2*ZVPICE*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + PAP(JL, JK)*YRECLDP%RCL_APB3*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)**3
            ZCORRFAC = (1.0 / ZRHO)**0.5
            ZCORRFAC2 = ((ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) / 273.0)**1.5)*(393.0 / (ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL) + 120.0))
            
            ZPR02 = ZRHO*ZPRECLR*YRECLDP%RCL_CONST1S / (ZTCG*ZFACX1S)
            
            ZTERM1 = (ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL) - ZQE)*ZSTACK(IND_ZTP1 + (JK-1)*KLON + JL)**2*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2S*ZFACX1S /  &
            & (ZRHO*ZAPLUSB*ZSTACK(IND_ZQSICE + (JK-1)*KLON + JL))
            ...
            ZEVAP = MIN(ZEVAP, ZSTACK(IND_ZQX + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL))
            ...
            ! 2-flux treatment, abandoned due to the new prognostic precip
            !-------------------------------------------------------------
            ZCOVPTOT = MAX(YRECLDP%RCOVPMIN, ZCOVPTOT - MAX(0.0_JPRB, (ZCOVPTOT - ZSTACK(IND_ZA + (JK-1)*KLON + JL))*ZEVAP / ZSTACK(IND_ZQX + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL)))
        ...
        !---------------------------
        ZANEW = (ZSTACK(IND_ZA + (JK-1)*KLON + JL) + ZSOLAC) / (1.0_JPRB + ZSOLAB)
        ZANEW = MIN(ZANEW, 1.0_JPRB)
        IF (ZANEW < YRECLDP%RAMIN) ZANEW = 0.0_JPRB
        ZDA = ZANEW - ZSTACK(IND_ZAORIG + (JK-1)*KLON + JL)
        !---------------------------------
...
!$claw nodep
        ...
        DO JM=1,NCLV
          ZMAX = MAX(ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL), ZEPSEC)
        ...
        DO JM=1,NCLV
          ...
          !---------------------------
          ZMM = MAX(ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL), ZEPSEC)
        ...
        DO JM=1,NCLV
          ...
          END DO
          ZQXN(JM) = ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) + ZEXPLICIT
        END DO
        ...
        DO JM=1,NCLV
          ZQXNM1(JM) = ZQXN(JM)
          ZSTACK(IND_ZQXN2D + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) = ZQXN(JM)
        END DO
        ...
        DO JM=1,NCLV
          ZSTACK(IND_ZPFPLSX + (JM-1)*KLON*(KLEV+1) + (JK+1-1)*KLON + JL) = ZFALLSINK(JM)*ZQXN(JM)*ZRDTGDP
        END DO
        
        ! Ensure precipitation fraction is zero if no precipitation
        ZQPRETOT = ZSTACK(IND_ZPFPLSX + (NCLDQS-1)*KLON*(KLEV+1) + (JK+1-1)*KLON + JL) + ZSTACK(IND_ZPFPLSX + (NCLDQR-1)*KLON*(KLEV+1) + (JK+1-1)*KLON + JL)
        IF (ZQPRETOT < ZEPSEC) THEN
          ZCOVPTOT = 0.0_JPRB
        END IF
        ...
        DO JM=1,NCLV - 1
          ...
          IF (IPHASE(JM) == 1) THEN
            TENDENCY_LOC_T(JL, JK) = TENDENCY_LOC_T(JL, JK) + RALVDCP*(ZQXN(JM) - ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZFLUXQ(JM))*ZQTMST
          END IF
          
          IF (IPHASE(JM) == 2) THEN
            TENDENCY_LOC_T(JL, JK) = TENDENCY_LOC_T(JL, JK) + RALSDCP*(ZQXN(JM) - ZSTACK(IND_ZQX + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZFLUXQ(JM))*ZQTMST
          END IF
          ...
          !----------------------------------------------------------------------
          TENDENCY_LOC_CLD(JL, JK, JM) = TENDENCY_LOC_CLD(JL, JK, JM) + (ZQXN(JM) - ZSTACK(IND_ZQX0 + (JM-1)*KLON*KLEV + (JK-1)*KLON + JL))*ZQTMST
        ...
        ! 6.2 Humidity budget
        !----------------------
        TENDENCY_LOC_q(JL, JK) = TENDENCY_LOC_Q(JL, JK) + (ZQXN(NCLDQV) - ZSTACK(IND_ZQX + (NCLDQV-1)*KLON*KLEV + (JK-1)*KLON + JL))*ZQTMST
      ...
      DO JK=1,KLEV + 1
        PFPLSL(JL, JK) = ZSTACK(IND_ZPFPLSX + (NCLDQR-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL) + ZSTACK(IND_ZPFPLSX + (NCLDQL-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL)
        PFPLSN(JL, JK) = ZSTACK(IND_ZPFPLSX + (NCLDQS-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL) + ZSTACK(IND_ZPFPLSX + (NCLDQI-1)*KLON*(KLEV+1) + (JK-1)*KLON + JL)
      END DO
      ...
      DO JK=1,KLEV
        ...
        PFSQLF(JL, JK + 1) = PFSQLF(JL, JK + 1) + (ZSTACK(IND_ZQXN2D + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX0 + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL) + PVFL(JL, JK)*PTSPHY -  &
        & ZALFAW*PLUDE(JL, JK))*ZGDPH_R
        ! liquid, negative numbers
        PFCQLNG(JL, JK + 1) = PFCQLNG(JL, JK + 1) + ZSTACK(IND_ZLNEG + (NCLDQL-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZGDPH_R
        
        ! liquid, vertical diffusion
        PFSQLTUR(JL, JK + 1) = PFSQLTUR(JL, JK + 1) + PVFL(JL, JK)*PTSPHY*ZGDPH_R
        
        ! Rain, LS scheme
        PFSQRF(JL, JK + 1) = PFSQRF(JL, JK + 1) + (ZSTACK(IND_ZQXN2D + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX0 + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL))*ZGDPH_R
        ! rain, negative numbers
        PFCQRNG(JL, JK + 1) = PFCQRNG(JL, JK + 1) + ZSTACK(IND_ZLNEG + (NCLDQR-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZGDPH_R
        
        ! Ice , LS scheme minus detrainment
        PFSQIF(JL, JK + 1) = PFSQIF(JL, JK + 1) + (ZSTACK(IND_ZQXN2D + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX0 + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL) + PVFI(JL, JK)*PTSPHY -  &
        & (1.0_JPRB - ZALFAW)*PLUDE(JL, JK))*ZGDPH_R
        ! ice, negative numbers
        PFCQNNG(JL, JK + 1) = PFCQNNG(JL, JK + 1) + ZSTACK(IND_ZLNEG + (NCLDQI-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZGDPH_R
        
        ! ice, vertical diffusion
        PFSQITUR(JL, JK + 1) = PFSQITUR(JL, JK + 1) + PVFI(JL, JK)*PTSPHY*ZGDPH_R
        
        ! snow, LS scheme
        PFSQSF(JL, JK + 1) = PFSQSF(JL, JK + 1) + (ZSTACK(IND_ZQXN2D + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL) - ZSTACK(IND_ZQX0 + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL))*ZGDPH_R
        ! snow, negative numbers
        PFCQSNG(JL, JK + 1) = PFCQSNG(JL, JK + 1) + ZSTACK(IND_ZLNEG + (NCLDQS-1)*KLON*KLEV + (JK-1)*KLON + JL)*ZGDPH_R
      END DO
  ...
  END SUBROUTINE CLOUDSC_SCC_RAW_STACK
  
END MODULE CLOUDSC_GPU_OMP_SCC_RAW_STACK_MOD
