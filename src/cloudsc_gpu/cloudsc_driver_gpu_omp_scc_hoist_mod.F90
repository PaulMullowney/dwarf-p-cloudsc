...
CONTAINS
  ...
  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_HOIST( &
    ...
    REAL(KIND=JPRB), ALLOCATABLE :: ZSOLQA(:,:,:,:)
    REAL(KIND=JPRB), ALLOCATABLE :: ZSOLQB(:,:,:,:)
    REAL(KIND=JPRB), ALLOCATABLE :: ZQLHS(:,:,:,:)
    ...
    ALLOCATE(ZSOLQA(NPROMA,NCLV,NCLV,NGPBLKS))
    ALLOCATE(ZSOLQB(NPROMA,NCLV,NCLV,NGPBLKS))
    ALLOCATE(ZQLHS(NPROMA,NCLV,NCLV,NGPBLKS))
...
!$omp parallel do
      ...
      DO JL=1,ICEND
        CALL CLOUDSC_SCC_HOIST &
         ...
         & ZSOLQA(:,:,:,IBL), ZSOLQB(:,:,:,IBL), ZQLHS(:,:,:,IBL), &
         & JL=JL)
      ENDDO
#ifdef HAVE_OMP_TARGET_LOOP_CONSTRUCT
!$omp end loop
