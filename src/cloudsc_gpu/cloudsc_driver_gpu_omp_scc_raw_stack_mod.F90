...
MODULE CLOUDSC_DRIVER_GPU_OMP_SCC_RAW_STACK_MOD
  ...
  USE CLOUDSC_GPU_OMP_SCC_RAW_STACK_MOD, ONLY: CLOUDSC_SCC_RAW_STACK
  
  IMPLICIT NONE
  
  CONTAINS

  SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_RAW_STACK (NUMOMP, NPROMA, NLEV, NGPTOT, NGPTOTG, NGPBLKS, KFLDX, PTSPHY, PT, PQ, &
...
#else
!$omp target teams distribute thread_limit(256) !private(YLSTACK_L) 
#endif 
...
!$omp parallel do
    ...
    DO JL=1,ICEND
      ...
      CALL CLOUDSC_SCC_RAW_STACK(1, ICEND, NPROMA, NLEV, PTSPHY, PT(:, :, IBL), PQ(:, :, IBL), BUFFER_TMP(:, :, 1, IBL),  &
...
!$omp end target data
    ...
    DEALLOCATE (ZSTACK)
  END SUBROUTINE CLOUDSC_DRIVER_GPU_SCC_RAW_STACK
  
END MODULE CLOUDSC_DRIVER_GPU_OMP_SCC_RAW_STACK_MOD
