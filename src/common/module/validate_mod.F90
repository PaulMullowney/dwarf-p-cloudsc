...
MODULE VALIDATE_MOD
  ...
  INTERFACE VALIDATE
     PROCEDURE VALIDATE_L1, VALIDATE_I1, VALIDATE_R1, VALIDATE_R2, VALIDATE_R3, VALIDATE_R2_NEW, VALIDATE_R3_NEW
...
CONTAINS
  ...
  SUBROUTINE VALIDATE_R2_NEW(NAME, REF, FIELD, NLON, NLEV, NDIM, NGPTOT, NBLOCKS, NGPTOTG, INDEX)
    ! Computes and prints errors "in the L1 norm sense"
    CHARACTER(*), INTENT(IN) :: NAME
    REAL(KIND=JPRB), INTENT(INOUT) :: REF(:,:,:,:), FIELD(:,:,:,:)
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NLEV, NDIM, NBLOCKS, NGPTOT, INDEX
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    ...
    DO B=1, NBLOCKS
      BSIZE = MIN(NLON, NGPTOT - (B-1)*NLON)  ! Field block size
      ZMINVAL(1) = MIN(ZMINVAL(1),MINVAL(FIELD(1:BSIZE,:,INDEX,B)))
      ZMAX_VAL_ERR(1) = MAX(ZMAX_VAL_ERR(1),MAXVAL(FIELD(1:BSIZE,:,INDEX,B)))
      DO JL=1, NLEV
        DO JK=1, bsize
          ! Difference against reference result in one-norm sense
          ZDIFF = ABS(FIELD(JK,JL,INDEX,B) - REF(JK,JL,INDEX,B))
          ZMAX_VAL_ERR(2) = MAX(ZMAX_VAL_ERR(2),ZDIFF)
          ZSUM_ERR_ABS(1) = ZSUM_ERR_ABS(1) + ZDIFF
          ZSUM_ERR_ABS(2) = ZSUM_ERR_ABS(2) + ABS(REF(JK,JL,INDEX,B))
        ENDDO
      END DO
    END DO
    ...
    IF (IRANK == 0) THEN
      CALL ERROR_PRINT(NAME, ZMINVAL(1), ZMAX_VAL_ERR(1), ZMAX_VAL_ERR(2), &
        & ZSUM_ERR_ABS(1), ZSUM_ERR_ABS(2), ZAVGPGP, NDIM=2)
    END IF
  END SUBROUTINE VALIDATE_R2_NEW
  ...
  SUBROUTINE VALIDATE_R3_NEW(NAME, REF, FIELD, NLON, NLEV, NDIM, NGPTOT, NBLOCKS, NGPTOTG, I0, I1)
    ! Computes and prints errors "in the L1 norm sense"
    CHARACTER(*), INTENT(IN) :: NAME
    REAL(KIND=JPRB), INTENT(INOUT) :: REF(:,:,:,:), FIELD(:,:,:,:)
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NLEV, NDIM, NBLOCKS, NGPTOT, I0, I1
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
    ...
    DO B=1, NBLOCKS
      BSIZE = MIN(NLON, NGPTOT - (B-1)*NLON)  ! Field block size
      ZMINVAL(1) = MIN(ZMINVAL(1),MINVAL(FIELD(1:BSIZE,:,I0:I1,B)))
      ZMAX_VAL_ERR(1) = MAX(ZMAX_VAL_ERR(1),MAXVAL(FIELD(1:BSIZE,:,I0:I1,B)))
      DO JM=I0, I1
        DO JL=1, NLEV
          DO JK=1, bsize
            ! Difference against reference result in one-norm sense
    ...
    IF (IRANK == 0) THEN
      CALL ERROR_PRINT(NAME, ZMINVAL(1), ZMAX_VAL_ERR(1), ZMAX_VAL_ERR(2), &
        & ZSUM_ERR_ABS(1), ZSUM_ERR_ABS(2), ZAVGPGP, NDIM=3)
    END IF
  END SUBROUTINE VALIDATE_R3_NEW
