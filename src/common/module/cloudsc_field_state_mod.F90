...
MODULE CLOUDSC_FIELD_STATE_MOD
  ...
  TYPE CLOUDSC_FIELD_STATE
    ...
    CLASS(FIELD_3RB), POINTER :: F_PAPH, F_PCOVPTOT
...
!$omp end parallel do
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_LOAD(SELF, NPROMA, NGPTOT, NGPTOTG, USE_PACKED)
    ...
    ALLOCATE(SELF%FIELDS_RDONLY(24))
    ALLOCATE(SELF%FIELDS_RWONLY(14))

    IF (LLPACKED) THEN
      PRINT *, "Packed storage option not yet enabled"
      ERROR STOP
...
!      SELF%F_PFHPSN => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RWONLY, IDX=14)
    ...
    ELSE
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(1)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(2)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(3)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(4)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(5)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(6)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(7)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(8)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(9)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(10)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(11)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(12)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(13)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(14)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(15)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(16)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(17)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(18)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(19)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(20)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(21)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(22)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(23)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%FIELDS_RDONLY(24)%PTR, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)

      ! Custom fields that do not share shape or data type with the other blocks
      CALL FIELD_NEW(SELF%F_PAPH, UBOUNDS=[NPROMA,SELF%KLEV+1,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%F_PLSM, UBOUNDS=[NPROMA,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
      CALL FIELD_NEW(SELF%F_LDCUM, UBOUNDS=[NPROMA,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=.TRUE.)
      CALL FIELD_NEW(SELF%F_KTYPE, UBOUNDS=[NPROMA,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0_JPIM)
      CALL FIELD_NEW(SELF%F_PCLV, UBOUNDS=[NPROMA,SELF%KLEV,NCLV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
    ...
    CALL FIELD_NEW(SELF%TENDENCY_TMP%F_T, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
    CALL FIELD_NEW(SELF%TENDENCY_TMP%F_A, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
    CALL FIELD_NEW(SELF%TENDENCY_TMP%F_Q, UBOUNDS=[NPROMA,SELF%KLEV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
    CALL FIELD_NEW(SELF%TENDENCY_TMP%F_CLD, UBOUNDS=[NPROMA,SELF%KLEV,NCLV,SELF%NBLOCKS], PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)

    CALL LOAD_AND_EXPAND_FIELD_3D('PLCRIT_AER', SELF%FIELDS_RDONLY(20)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PICRIT_AER', SELF%FIELDS_RDONLY(21)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PRE_ICE', SELF%FIELDS_RDONLY(22)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PCCN', SELF%FIELDS_RDONLY(23)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PNICE', SELF%FIELDS_RDONLY(24)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    ...
    CALL LOAD_AND_EXPAND_FIELD_3D('PLUDE', SELF%FIELDS_RDONLY(14)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PSNDE', SELF%FIELDS_RDONLY(15)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PMFU', SELF%FIELDS_RDONLY(16)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PMFD', SELF%FIELDS_RDONLY(17)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PA', SELF%FIELDS_RDONLY(18)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_4D('PCLV', SELF%F_PCLV, KLON, SELF%KLEV, NCLV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
    CALL LOAD_AND_EXPAND_FIELD_3D('PSUPSAT', SELF%FIELDS_RDONLY(19)%PTR, KLON, SELF%KLEV, NPROMA, NGPTOT, SELF%NBLOCKS, NGPTOTG)
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_VALIDATE(SELF, NPROMA, NGPTOT, NGPTOTG)
    ...
    CALL VALIDATE('PLUDE', PLUDE, SELF%FIELDS_RDONLY(14)%PTR%PTR, NPROMA, SELF%KLEV, NGPTOT, NBLOCKS, NGPTOTG)
