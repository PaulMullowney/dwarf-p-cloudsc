...
MODULE CLOUDSC_FIELD_STATE_MOD
  ...
  USE FIELD_BUFFER_MODULE, ONLY: FIELD_4RB_BUFFER
  ...
  TYPE CLOUDSC_FIELD_STATE
    ...
    TYPE(FIELD_4RB_BUFFER), ALLOCATABLE :: DATA_RDONLY
    TYPE(FIELD_4RB_BUFFER), ALLOCATABLE :: DATA_RWONLY
...
!$omp end parallel do
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_LOAD(SELF, NPROMA, NGPTOT, NGPTOTG, USE_PACKED)
    ...
    IF (LLPACKED) THEN
      ...
      NFIELDS = 23
      ALLOCATE(SELF%DATA_RDONLY)
      CALL SELF%DATA_RDONLY%INIT(NFIELDS, SELF%FIELDS_RDONLY, UBOUNDS=[NPROMA, SELF%KLEV, NFIELDS, SELF%NBLOCKS], &
      ...
      NFIELDS = 14
      ALLOCATE(SELF%DATA_RWONLY)
      CALL SELF%DATA_RWONLY%INIT(NFIELDS, SELF%FIELDS_RWONLY, UBOUNDS=[NPROMA, SELF%KLEV+1, NFIELDS, SELF%NBLOCKS], &
      &                          PERSISTENT=.TRUE., INIT_VALUE=0._JPRB)
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_FINALIZE(SELF, USE_PACKED)
    ...
    IF(USE_PACKED)THEN
      CALL SELF%DATA_RDONLY%FINAL()
      CALL SELF%DATA_RWONLY%FINAL()

      DEALLOCATE(SELF%DATA_RDONLY)
      DEALLOCATE(SELF%DATA_RWONLY)
